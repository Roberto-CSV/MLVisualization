...
class MachineLearningService:
    ...
    def train_model(self, model: MachineLearningModel) -> None:
        transformed_data: pd.DataFrame = self.__transform_data__(self.data_frame)
        x_train: pd.DataFrame = transformed_data.drop(['Segmentation'], axis=1)
        y_train: pd.Series = transformed_data['Segmentation']
    ...
    def model_predict(self, model: MachineLearningModel):
        transformation_equivalence: dict = self.__get_transformation_equivalence__(self.data_frame)
        data_to_predict: pd.DataFrame = PandasUtil.read_file(self.document_data_type, './data/prediction.csv')
        data_to_predict = self.__transform_data__(data_to_predict)
        true_prediction: pd.DataFrame = PandasUtil.read_file(self.document_data_type, './data/prediction_y.csv')
        true_prediction['Segmentation'] = true_prediction['Segmentation'].replace(transformation_equivalence.get('Segmentation'))
        true_prediction = true_prediction['Segmentation'].values
        model_prediction = None
        if(model.type.__eq__(AlgorithmsTypesConstant.REGRESSION)):
            model_prediction = self.__get_regression_model__(model=model, result_type=ResultTypes.PREDICT, data_to_predict=data_to_predict)
            mae = mean_absolute_error(true_prediction, model_prediction)
            mse = mean_squared_error(true_prediction, model_prediction)
            rmse = mean_squared_error(true_prediction, model_prediction, squared=False)  # raíz cuadrada de MSE
            r2 = r2_score(true_prediction, model_prediction)
            ...
            model_prediction = self.__get_classification_model__(model=model, result_type=ResultTypes.PREDICT, data_to_predict=data_to_predict)
            print(classification_report(true_prediction, model_prediction, zero_division=1))
